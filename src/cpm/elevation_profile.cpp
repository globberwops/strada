...
auto ElevationProfile::Evaluate(RoadId road, double s, double t) const noexcept -> VerticalProfile {
  ...
  bool has_css = (road_has_css_[road_idx] != 0U);
  ...
  double d_elev = 0.0;
  uint32_t elev_first_idx = elevation_.road_elevation_first_idx[road_idx];
  uint32_t elev_count = elevation_.road_elevation_count[road_idx];
  if (elev_count > 0) {
    ...
    double ds_poly = s - polynomials_.s_start[active_idx];
  ...
  double shape_h = EvaluateShapeHeight(road, s, t);
  double shape_grad = EvaluateShapeTGradient(road, s, t);
...
auto ElevationProfile::EvaluateShapeHeight(RoadId road, double s, double t) const noexcept -> double {
  ...
  uint32_t first_idx = shapes_.road_shape_first_idx[road_idx];
  uint32_t count = shapes_.road_shape_count[road_idx];
  ...
  double h1 = EvaluateGroupHeight(shapes_, *g1, t);
  if (g1->s == g2->s) {
    return h1;
  }
  double h2 = EvaluateGroupHeight(shapes_, *g2, t);
  double f = (s - g1->s) / (g2->s - g1->s);
...
auto ElevationProfile::EvaluateShapeTGradient(RoadId road, double s, double t) const noexcept -> double {
  ...
  double g1_val = EvaluateGroupTGradient(shapes_, *g1, t);
  if (g1->s == g2->s) {
    return g1_val;
  }
  double g2_val = EvaluateGroupTGradient(shapes_, *g2, t);
  double f = (s - g1->s) / (g2->s - g1->s);
