...
void Camera::ZoomAt(float screen_x, float screen_y, float factor) noexcept {
  ...
  float rad = rotation * std::numbers::pi_v<float> / 180.0F;
...
auto CalculateScaleLength(double zoom) noexcept -> double {
  ...
  double l_ideal = kIdealScaleWidth / zoom;
  double exponent = std::floor(std::log10(l_ideal));
  double base = std::pow(kBase10, exponent);
  ...
  for (int decade = -1; decade <= 1; ++decade) {
    double scale_mult = base * std::pow(kBase10, decade);
    for (double f : kFactors) {
      ...
      if (width >= kMinScaleWidth && width <= kMaxScaleWidth) {
        found_in_range = true;
        double dist = std::abs(width - kIdealScaleWidth);
  ...
  if (!found_in_range) {
    ...
    for (int decade = -2; decade <= 2; ++decade) {
      ...
      for (double f : kFactors) {
        double candidate = f * scale_mult;
        double width = candidate * zoom;
        double diff = std::abs(width - kIdealScaleWidth);
        if (diff < min_diff) {
          min_diff = diff;
          best_val = candidate;
        }
