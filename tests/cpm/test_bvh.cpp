...
TEST(BvhTest, SingleNodeConstruction) {
  // Arrange
  std::vector<uint32_t> prim_indices = {0};
  std::vector<BvhPrimitiveInfo> temp_primitives = {{.road_idx = 0, .segment_idx = 0}};
  std::vector<Aabb> temp_aabbs = {{.min_x = 10.0, .min_y = 20.0, .max_x = 30.0, .max_y = 40.0}};

  // Act
  auto bvh = Bvh::Build(prim_indices, temp_primitives, temp_aabbs);

  // Assert
  ASSERT_EQ(bvh.Nodes().size(), 1);
...
TEST(BvhTest, RecursiveSplitting) {
  // Arrange
...
TEST(BvhTest, TraversalAndPruning) {
  // Arrange
  std::vector<uint32_t> prim_indices = {0, 1, 2, 3, 4};
  std::vector<BvhPrimitiveInfo> temp_primitives = {{.road_idx = 0, .segment_idx = 0},
                                                   {.road_idx = 1, .segment_idx = 0},
  ...
  bvh.Query(0.5, 0.5, [&](const BvhPrimitiveInfo& prim, double current_min_dist) -> std::optional<double> {
