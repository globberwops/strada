...
* **Topological Snapping (Snapping Fast-Path)**: When the vehicle exits the current road boundary during inverse queries, the snapping logic uses pre-compiled road connectivity (neighbor lists) to resolve the next road before resorting to a global spatial search.
* **Priority Fallback Path**: The topologically connected junction path with the smallest heading change (straightest path), used as the primary successor check during route-free snapping.
...
* **`QueryContext`**: A small consumer-owned value type that carries the state needed to exploit *temporal coherence* across queries on the same road (last `RoadId`, last `s`-range, last segment, last BVH node). One instance per thread of execution, typically declared `thread_local` at the top of the simulation loop. Default-constructed `QueryContext` is "empty" — the first query is a full lookup, subsequent queries in temporal proximity hit the fast path. v1 is single-query only; batch APIs are not exposed.
  * **Route-Guided Snapping**: When `QueryContext::active_route` is populated with a sequence of `RoadId`s, the snapping logic limits candidate search roads to the current and next route roads, automatically advancing the route index on transition.
  * **Route-Free Snapping**: When no route is specified, snapping uses topological connectivity (predecessors, successors, and sibling connecting roads at junctions) to snap to candidate roads.
